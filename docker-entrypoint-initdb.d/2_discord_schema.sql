...
CREATE TABLE `players_submitted` (
  `ID` int NOT NULL,
  `Players` text
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`extreme4all`@`%` SQL SECURITY DEFINER VIEW `verified_players`  AS SELECT `dv`.`Entry` AS `Entry`, `dv`.`Discord_id` AS `Discord_id`, `dv`.`Player_id` AS `Player_id`, `dv`.`primary_rsn` AS `primary_rsn`, `dv`.`Code` AS `Code`, `dv`.`Verified_status` AS `Verified_status`, `pl`.`name` AS `name` FROM (`discordVerification` `dv` join `playerdata`.`Players` `pl` on((`dv`.`Player_id` = `pl`.`id`))) ;
